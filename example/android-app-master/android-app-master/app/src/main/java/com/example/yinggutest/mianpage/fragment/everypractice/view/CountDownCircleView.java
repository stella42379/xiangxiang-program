...
public class CountDownCircleView extends View {
    ...
    public CountDownCircleView(Context context, AttributeSet attrs, int defStyleAttr) {
      ...
      //  mRingProgessTextSize = a.getDimensionPixelSize(R.styleable.CountDownView_progressTextSize, DisplayUtils.sp2px(context, 20));
        mRingProgessTextSize = a.getDimensionPixelSize(R.styleable.CountDownView_progressTextSize,  30);
        mProgessTextColor = a.getColor(R.styleable.CountDownView_progressTextColor, context.getResources().getColor(R.color.white));
        //倒计时 时间
        int countdown = ConstantData.answerId.length *120;
        mCountdownTime = a.getInteger(R.styleable.CountDownView_countdownTime,countdown);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int time = mCountdownTime - (int) (mCurrentProgress / 360f * mCountdownTime);
        /*int minute = time/60;
        int second = time%60;
        String text = minute+"分"+second+"秒";*/
        String text = time+"";
    ...
    public void startCountDown() {
        ...
        valueAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //倒计时结束回调
                if (mListener != null) {
                    mListener.countDownFinished(IsStop);
                }
                setClickable(true);
            }
    ...
    public  void stopCountDown(boolean isStop){
        this.IsStop = isStop;
    ...
    public interface OnCountDownFinishListener {
        void countDownFinished(boolean isStop);
    }
