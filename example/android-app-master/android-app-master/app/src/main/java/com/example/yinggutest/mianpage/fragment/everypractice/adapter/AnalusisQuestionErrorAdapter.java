...
import com.example.yinggutest.AnalysisErrorQuestion;
...
public class AnalusisQuestionErrorAdapter extends PagerAdapter {

    private static final String TAG = "AnalusisQuestionErrorAd";
    AnalysisErrorQuestion mContext;
    // 传递过来的页面view的集合
    List<View> viewItems;
    // 每个item的页面view
    View convertView;
    // 传递过来的所有数据
    List<ErrorQuestionInfo> dataItems;
    ...
    public AnalusisQuestionErrorAdapter(AnalysisErrorQuestion context, List<View> viewItems, List<ErrorQuestionInfo> dataItems, String imgServerUrl) {
        mContext = context;
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        String option_type = dataItems.get(position).getOptionType();
        ...
        String questionSelect = dataItems.get(position).getQuestionSelect();
        if (questionSelect.equals("A")) {
            holder.ivA.setImageResource(R.mipmap.ic_practice_test_wrong);
        }else if(questionSelect.equals("B")){
            holder.ivB.setImageResource(R.mipmap.ic_practice_test_wrong);
        }else if(questionSelect.equals("C")){
            holder.ivC.setImageResource(R.mipmap.ic_practice_test_wrong);
        }else if(questionSelect.equals("D")){
            holder.ivD.setImageResource(R.mipmap.ic_practice_test_wrong);
        }

        String correctAnswer = dataItems.get(position).getQuestionAnswer();
    ...
    class LinearOnClickListener implements View.OnClickListener {
        ...
        private AnalusisQuestionErrorAdapter.ViewHolder viewHolder;

        public LinearOnClickListener(int position, boolean mIsNext, int position1, AnalusisQuestionErrorAdapter.ViewHolder viewHolder) {
            mPosition = position;
